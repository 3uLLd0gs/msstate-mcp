...
**26 MCP tools across 7 MSU content domains.** Current version: **v1.2.3** (2026-05-19). The hosted Cloudflare Worker ships routing instructions over MCP — Claude and ChatGPT both pick the right tool without per-session prompting.
...
4. Save. You should see **26 tools** appear
...
**4.** Verify the `msstate-policies` server shows **26 tools**. First call takes ~5 seconds (cold fetch); later calls reuse cached data.
