...
- [x] Hybrid retrieval (BM25 + pre-computed embeddings, RRF fusion) **live** in `src/search.ts`. **Comparative eval done 2026-05-08** (see [`eval/COMPARATIVE-2026-05-08.md`](./msstate-policies/eval/COMPARATIVE-2026-05-08.md)): BM25-only and embed-only tied at composite 86/88; hybrid (RRF) underperformed at 84/88 (lost the underage-drinking-at-tailgate case to embedding-side concept drift). Per Sprint 2 task 2.9, recommendation is to default to `bm25` mode. Env-var `MSSTATE_POLICIES_RETRIEVAL=bm25|embed|hybrid` (commit `f5157e3`) lets callers override. Default flip is left to the maintainer.
...
- [ ] 50 eval questions complete with ≥ 15 externally sourced (Tiger T2). *(50 questions exist in `eval/questions.jsonl`. `eval/SOURCES.md` now scaffolded with the current author-written provenance + the Tiger T2 sourcing template; the **≥ 15 externally sourced** bucket still has 0 entries — needs human sourcing from r/msstate, MSU advising FAQs, or a blind-author cohort. The corpus rule prevents AI-generated questions.)*
- [x] First eval run committed under `eval/eval-{date}.json` with three sub-metric scores. *(Multiple runs committed; latest is `eval-2026-05-08-k5-sonnet-4-6.json`.)* Manual review of 100% of LLM-judge answers: **done 2026-05-08**, see [`eval/REVIEW-2026-05-08.md`](./msstate-policies/eval/REVIEW-2026-05-08.md). No weak passes; 86/88 holds up under scrutiny.
...
- [x] Disk cache via `env-paths` (cross-platform). *(Shipped 2026-05-08, commit `9018ae4`. `TTLCache<T>` accepts `{ttlMs, persistKey, persistDir?}`; policy-body cache opts in via `MSSTATE_POLICIES_CACHE=disk`. 6 new tests — backward-compat, write/reload, expiry-on-load, clear unlinks file, cold-start, corrupt-file resilience. Index cache stays in-memory.)* WAF detection battle-test still pending — needs hand-run against the live MSU site.
